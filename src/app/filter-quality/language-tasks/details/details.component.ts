...
import {
  IDatePickerConfig,
  ECalendarValue,
} from 'src/app/shared-components/datepicker';
...
export class DetailsComponent implements OnInit {
  ...
  async ngOnInit(): Promise<void> {
    ...
    if (taskId) {
      ...
      this.store.dispatch(
        LanguageTasksActions.requestLanguageTaskDetails({ params })
      );
      this.store
        .pipe(select(LanguageTasksSeletor._getLanguageTaskDetails))
        .subscribe((languageTasks: TaskOutput) => {
          const tasks =
            languageTasks && languageTasks.items ? languageTasks.items : [];
          if (tasks.length > 0) {
            this.task = tasks.find(
              (item) => item.queueItem.contentId == taskId
            ); //tasks[0];
    ...
    if (this.pageType === 1) {
      const formData: TaskInput = {
        ...
        data: {
          ...
          dueDate: moment(
            this.taskFormData.data.dueDate,
            'MM-DD-YYYY'
          ).valueOf(),
          additionalProp1: {},
        },
  ...
  async onStart(): Promise<void> {
    let me = await this.userService.me();
    const body = {
      moderatorId: me.moderatorId,
    };
    this.apiService
      .updateItemCheckout('task', this.task.queueItem.contentId, body)
      .subscribe((resp) => {
        if (resp.success) {
  ...
  onStop(): void {
    this.apiService
      .deleteItemCheckout('task', this.task.queueItem.contentId)
      .subscribe((resp) => {
        if (resp.success) {

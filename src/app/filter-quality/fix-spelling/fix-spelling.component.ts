...
export class FixSpellingComponent extends BaseQueueComponent<FixSpellingItem> {
  ...
  async completeItems() {
    ...
    this.queueItems = this.queueItems.map((item) => {
      ...
      const tags = new Set(item.changes.tags || []);

      // Populate the FIXED.ALT_SENSE tag and Rules API update queue
      if (
        item.changes.extras.altSenses?.length > 0 &&
      ...
      if (
        item.changes.extras.altSpellings?.length > 0 &&
      ...
      rulesApiQueue.push([
        item.data.text,
        item.changes.extras.altSenses,
        item.changes.extras.altSpellings,
        queueItem.language.language,
        queueItem.clientId,
      ]);

      item.changes.tags = Array.from(tags);

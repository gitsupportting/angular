...
describe('PaginationControlsDirective:', () => {
  ...
  it('should warn on interaction when an unknown id is used', fakeAsync(() => {
    overrideTemplate(
      ...
      `
                   ...
                   <mod-pagination-template id="unknown_id" #pApi="paginationApi">
                     <a class="prev" (click)="pApi.previous()">prev</a>
                     <a class="next" (click)="pApi.next()">next</a>
  ...
  it('should allow an id of 0 to be used', fakeAsync(() => {
    overrideTemplate(
      ...
      `
                   ...
                   <mod-pagination-template [id]="0" #pApi="paginationApi">
                     <a class="prev" (click)="pApi.previous()">prev</a>
                     <a class="next" (click)="pApi.next()">next</a>
                   </mod-pagination-template>`
    );
  ...
  describe('template api', () => {
    function getControlsDirective(
      ...
      const controlsDirective: PaginationControlsDirective = fixture.debugElement.query(
  ...
  describe('bounds correction', () => {
    it('corrects the currentPage to be within bounds when in server-side mode', fakeAsync(() => {
      ...
      tick();
      expect(testCmpInstance.pageChanged).toHaveBeenCalledTimes(0);
      expect(testCmpInstance.pageChangedBoundsCorrection).toHaveBeenCalledTimes(
        1
      );
    ...
    >
      <div class="custom-template">
        <div
          ...
          [class.disabled]="pApi.isFirstPage()"
          *ngIf="pApi.directionLinks"
        >
          <span *ngIf="!pApi.isFirstPage()" (click)="pApi.previous()">back</span>
        </div>
        ...
        >
          <span (click)="pApi.setCurrent(page.value)">{{ page.label }}</span>
        </div>

        <div
          class="pagination-next"
          [class.disabled]="pApi.isLastPage()"
          *ngIf="pApi.directionLinks"
        >
          <span *ngIf="!pApi.isLastPage()" (click)="pApi.next()">forward</span>
        </div>
      </div>
    </mod-pagination-template>`,
})

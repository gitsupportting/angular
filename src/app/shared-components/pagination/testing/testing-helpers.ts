...
export function getListItemsText(fixture: ComponentFixture<any>): string[] {
  return getListItems(fixture).map((el) => el.textContent.trim());
}
...
export function getControlsDirective(
  ...
  return fixture.debugElement.query(By.css('mod-pagination-template'))
    .references.pApi;
}
...
export function getPageLinkItems(
  ...
  selector: string = 'mod-pagination-controls li',
  includeAll: boolean = false
): string[] {
  const all222 = fixture.debugElement.queryAll(By.css(selector));
 ...
 */
export function overrideTemplate<T>(
  component: Type<T>,

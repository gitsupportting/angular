...
describe('Component: DayCalendarComponent', () => {
  ...
  describe('should have the right CSS classes for', () => {
    ...
    const defaultCssClasses: { [klass: string]: boolean } = {
      ...
      'mod-prev-month': false,
      'mod-next-month': false,
      'mod-current-day': false,
    };
    ...
    it('the current day', () => {
      expect(
        ...
        ...defaultCssClasses,
        'mod-current-day': true,
      });
    });

    it('custom days', () => {
      component.componentConfig.dayBtnCssClassCallback = (day: Moment) =>
        'custom-class';
  ...
  describe('should have the correct weekday format', () => {
    ...
    it('weekdayFormatter', () => {
      ...
      expect(component.getWeekdayName(moment())).toBe(
        moment().day().toString()
      );
    });
  });

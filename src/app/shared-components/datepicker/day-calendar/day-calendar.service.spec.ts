...
describe('Service: Calendar', () => {
  ...
  it('should check isDateDisabled method', inject(
    ...
    (service: DayCalendarService) => {
      ...
      expect(
        service.isDateDisabled(moment('11-10-2016', 'DD-MM-YYYY'), config)
      ).toBe(true);
      expect(
        service.isDateDisabled(moment('12-10-2016', 'DD-MM-YYYY'), config)
      ).toBe(false);
      expect(
        service.isDateDisabled(moment('13-10-2016', 'DD-MM-YYYY'), config)
      ).toBe(false);
      expect(
        service.isDateDisabled(moment('14-10-2016', 'DD-MM-YYYY'), config)
      ).toBe(false);
      expect(
        service.isDateDisabled(moment('15-10-2016', 'DD-MM-YYYY'), config)
      ).toBe(true);
  ...
  it('should check getDayBtnText method', inject(
    ...
    (service: DayCalendarService) => {
      ...
      expect(
        service.getDayBtnText({ dayBtnFormatter: (m) => 'bla' }, date)
      ).toEqual('bla');
      expect(
    ...
    (service: DayCalendarService) => {
      ...
      expect(
        service.getDayBtnCssClass(
          { dayBtnCssClassCallback: (m) => 'class1 class2' },
          date
        )

...
export class TimeSelectComponent
  ...
  constructor(
    public timeSelectService: TimeSelectService,
  ...
  private handleConfigChange(config: SimpleChange) {
    if (config) {
      const prevConf: IDayCalendarConfigInternal = this.timeSelectService.getConfig(
        config.previousValue
      );
      const currentConf: IDayCalendarConfigInternal = this.timeSelectService.getConfig(
        config.currentValue
      );

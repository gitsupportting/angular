...
export class MonthCalendarComponent
  ...
  onLeftSecondaryNavClick(): void {
    ...
    const from = this.currentDateView.clone();
    this.currentDateView = this.currentDateView
      .clone()
      .subtract(navigateBy, 'year');
    const to = this.currentDateView.clone();
  ...
  getMonthBtnCssClass(month: IMonth): { [klass: string]: boolean } {
    const cssClass: { [klass: string]: boolean } = {
      'mod-selected': month.selected,
      'mod-current-month': month.currentMonth,
    };
  ...
  handleConfigChange(config: SimpleChange): void {
    if (config) {
      const prevConf: IMonthCalendarConfigInternal = this.monthCalendarService.getConfig(
        config.previousValue
      );
      const currentConf: IMonthCalendarConfigInternal = this.monthCalendarService.getConfig(
        config.currentValue
      );

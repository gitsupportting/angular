...
export class OptionButtonGroupComponent implements OnInit {
  ...
  getButtonClass(option: IOptionButton) {
    // check active button with the "multiple" params
    // add class bg-secondary for the default type
    if (
      this.multiple
        ? this.value.indexOf(option.value.toString()) > -1
        : this.value === option.value
    ) {
      return this.type === 'default' && 'bg-secondary text-on-secondary';
    }

    // if this button is not active
    // add the background transparent to the secondary type
    return this.type === 'secondary' && 'bg-transparent';
  }
}

...
export class BaseQueueService {
  ...
  loadItems(pageNumber = 1) {
    ...
    if (this._queueInfo) {
      return this.getReadonlyItems({
        offset: Math.max(this.resultsPerPage * (pageNumber - 1), 0),
      }).subscribe(() => {
    ...
    this.queueApiService
      ...
      .pipe(
        ...
        flatMap(() => {
          const offset = this.resultsPerPage * (pageNumber - 1);
          return this.getReadonlyItems({ offset });
        })
      )
      .subscribe(({ total }) => {
        this._queueState = QueueState.INITIAL;
  ...
  async completePage(updatedItems: AuditQueueItem[]) {
    ...
    await this.queueApiService.addDecisions(
      ...
      updatedItems.map((item) => {
        // Keep all non-label tags that were on the initial queue item
        const initialContentTypeTags = item.queueItem.tags.filter(
          (tag) => !labelTags.includes(tag)
        );

        const tags: string[] = item.changes.tags
          // Filter out tags that aren't available labels
          .filter((tag) => labelTags.includes(tag))
          // Merge all non-label tags that were on the initial item

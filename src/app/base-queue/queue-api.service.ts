...
import { ITopic } from '../shared-components/interfaces';
...
export class QueueApiService {
        ...
        map(({ total, items }) => {
          return {
            ...
            items: items.map(({ data, queueItem }) => {
              const topics: ITopic[] = Object.entries(queueItem.topics).map(
                ([id, risk]) => {
                  return { id: parseInt(id, 10), risk };
                }
              );
              const result: AuditQueueItem<T> = {
                data,
                queueItem: {
                  tags: [],
                  ...queueItem,
                ...
                changes: {
                  generalRisk: queueItem.topics['0'],
                  topics: topics.slice(),
                  tags: queueItem.tags?.slice() || [],
                  extras: {},
                },
              };
              return result;
  ...
  async addDecisions(
    queueName: string,
    seconds: number,
    queueItems: AuditQueueItem[]
  ): Promise<boolean> {
    const now = Date.now();
    const secondsPerItem = Math.round(seconds / queueItems.length);

    const res = await Promise.all(
      queueItems.map(async (item) => {
        const contentId = item.queueItem.contentId;
        ...
        return this.addDecision(queueName, contentId, {
          ...
          tags: item.changes.tags.filter(
            (tag) => !item.queueItem.tags.includes(tag)
          ),
          // Tags that are in initial item and not in the updated item
          tagsRemoved: item.queueItem.tags.filter(
            (tag) => !item.changes.tags.includes(tag)
          ),
          reviewedTopics: item.queueItem.topics.reduce((result, topic) => {
            result[topic.id] = topic.risk;
            return result;
          }, {}),
          correctTopics: item.changes.topics.reduce((result, topic) => {
            result[topic.id] = topic.risk;
            return result;
          }, {}),
